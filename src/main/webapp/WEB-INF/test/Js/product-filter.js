jQuery(document).ready(function(){
	var productCustomization = $('.qw-customization'),
		cart = $('.qw-cart'),
		animating = false;
	...
	$('body').on('click', function(event){
		//if user clicks outside the .qw-gallery list items - remove the .hover class and close the open ul.size/ul.color list elements
		if( $(event.target).is('body') || $(event.target).is('.qw-gallery') ) {
	...
	function initCustomization(items) {
		items.each(function(){
			var actual = $(this),
				...
				touchSettings = actual.next('.qw-customization-trigger');
	...
	function updateSlider(actual, index) {
		var slider = actual.parent('.qw-customization').prev('a').children('.qw-slider-wrapper'),
	...
	function resetCustomization(selectOptions) {
		...
		selectOptions.siblings('[data-type="select"]').removeClass('is-open').end().parents('.qw-single-item').addClass('hover').parent('li').siblings('li').find('.qw-single-item').removeClass('hover').end().find('[data-type="select"]').removeClass('is-open');
	}

	function deactivateCustomization() {
		productCustomization.parent('.qw-single-item').removeClass('hover').end().find('[data-type="select"]').removeClass('is-open');
	}
